...
def make_product(user, name='Producto', cost=1000, sale=1200):
    return Product.objects.create(
        contractor=user, name=name, unit='un', cost_price=cost, sale_price=sale
    )
...
class ProductCRUDTest(TestCase):
    ...
    def test_product_create_post(self):
        self.tc.post(reverse('product_create'), {
            'name': 'Cerámica', 'unit': 'un', 'category': 'ceramica',
            'cost_price': '5000', 'sale_price': '7000', 'sku': '', 'description': '',
        })
        self.assertEqual(Product.objects.filter(contractor=self.user).count(), 1)

    def test_product_edit(self):
        p = make_product(self.user, 'Viejo')
        self.tc.post(reverse('product_edit', args=[p.pk]), {
            'name': 'Nuevo', 'unit': 'un', 'category': 'materiales',
            'cost_price': '1000', 'sale_price': '1500', 'sku': '', 'description': '',
        })
        p.refresh_from_db()
