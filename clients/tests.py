...
class ClientCRUDViewTest(TestCase):
    ...
    def test_client_create_post(self):
        r = self.tc.post(reverse('client_create'), {
            'name': 'Pedro Pérez', 'phone': '123456', 'email': '', 'rut': '',
            'address': '', 'city': '', 'notes': '',
        })
    ...
    def test_client_edit(self):
        ...
        self.tc.post(reverse('client_edit', args=[c.pk]), {
            'name': 'Editado', 'phone': '2', 'email': '', 'rut': '',
            'address': '', 'city': '', 'notes': '',
        })
        c.refresh_from_db()
...
class ClientMultiTenantTest(TestCase):
    ...
    def test_user_a_cannot_edit_client_of_b(self):
        r = self.tc.post(reverse('client_edit', args=[self.client_b.pk]), {
            'name': 'Hack', 'phone': '0', 'email': '', 'rut': '',
            'address': '', 'city': '', 'notes': '',
        })
        self.assertEqual(r.status_code, 404)
