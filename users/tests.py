...
class PasswordResetFlowTest(TestCase):
    ...
    def test_confirm_valid_token_updates_password(self):
        uidb64, token = self._generate_token(self.user)
        r = self.tc.post(reverse('password_reset_confirm', args=[uidb64, token]), {
    ...
    def test_confirm_token_single_use(self):
        uidb64, token = self._generate_token(self.user)
        self.tc.post(reverse('password_reset_confirm', args=[uidb64, token]), {
            'new_password1': 'NuevaClave9876!',
            'new_password2': 'NuevaClave9876!',
        })
